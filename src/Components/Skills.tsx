  ...
  return (
    ...
    <div className='flex my-16'>
        <div className='w-2/4 flex justify-end grid grid-rows-3  grid-flow-col-dense gap-x-3 gap-y-6'>
            {new Array(7).fill(null).map((i,k)=><div key={k.toString()+"skill"} className="flex flex-col justify-center  items-center h-28 w-28 mx-8 rounded-full bg-[#f8f1ff]">
                <img src={require('../Images/img/js.png')} className="h-10 w-10 " />
                <p className='text-sm font-semibold mt-3'>30%</p>
            </div>)}
        </div>
        <div className='w-2/4 '>
        {new Array(3).fill(null).map((i,j)=> <div key={j+"designation"} className='flex justify-center gap-x-6 mb-20'>
